...
// ==================== LOG DE DEPURACIÓN ====================
console.log('\n' + '═'.repeat(60));
console.log('📊 CONTENT CONTROLLER - INICIALIZADO');
console.log('═'.repeat(60));
console.log(`📰 Total noticias simuladas: ${todasLasNoticias.length}`);
console.log(`⭐ Noticias importantes: ${noticiasSimuladas.length}`);
console.log(`📅 Rango de fechas: ${todasLasNoticias[todasLasNoticias.length-1]?.fechaFormateada} - ${todasLasNoticias[0]?.fechaFormateada}`);
console.log(`📝 Primera noticia: ${todasLasNoticias[0]?.titulo.substring(0, 60)}...`);
console.log('═'.repeat(60) + '\n');
...
class ContentController {
  ...
  async getContents(req, res) {
    try {
      ...
      // ==================== LOG DE DEPURACIÓN ====================
      console.log('\n🔍 [getContents] SOLICITUD RECIBIDA');
      console.log(`   📄 Página: ${queryPage}, Límite: ${queryLimit}`);
      console.log(`   🏷️ Type: ${type || 'no filtro'}, Status: ${status || 'no filtro'}`);
      console.log(`   🎮 Modo simulación: ${usarDatosSimulados ? 'ACTIVADO' : 'DESACTIVADO'}`);
      console.log(`   📊 Total noticias disponibles: ${todasLasNoticias.length}`);
      
      // MODO SIMULACIÓN
      if (usarDatosSimulados) {
        console.log('\n📡 [MODO SIMULACIÓN] Procesando solicitud...');
        ...
        if (type && type !== 'all') {
          ...
          console.log(`   🔍 Filtradas por type=${type}: ${antes} → ${filtradas.length}`);
        }
        
        if (status && status !== 'all') {
          const antes = filtradas.length;
          filtradas = filtradas.filter(n => n.status === status)
          console.log(`   🔍 Filtradas por status=${status}: ${antes} → ${filtradas.length}`);
        }
        ...
        console.log(`\n✅ [MODO SIMULACIÓN] Resultado:`);
        console.log(`   📦 Enviando ${contents.length} noticias de ${total} totales`);
        console.log(`   📍 Rango: ${start + 1} - ${Math.min(end, total)}`);
        if (contents.length > 0) {
          console.log(`   📰 Primera noticia: ${contents[0].titulo.substring(0, 80)}...`);
          console.log(`   ⭐ ¿Es importante?: ${contents[0].importante ? 'SÍ' : 'NO'}`);
        } else {
          console.log(`   ⚠️ No hay noticias para mostrar`);
        }
      ...
      console.log('\n📡 [MODO REAL] Conectando a base de datos...');
      const filters = {};
      ...
      console.log(`✅ [MODO REAL] Enviando ${result.contents?.length || 0} noticias`);
      console.log('═'.repeat(40) + '\n');
  ...
  async getContentBySlug(req, res) {
    try {
      ...
      if (usarDatosSimulados) {
        ...
        console.log(`✅ [getContentBySlug] Encontrado: ${content.titulo.substring(0, 60)}...`);
      ...
      console.log(`✅ [getContentBySlug] Encontrado desde DB: ${content?.title}`);
  ...
  async getContentById(req, res) {
    try {
      ...
      if (usarDatosSimulados) {
        ...
        console.log(`✅ [getContentById] Encontrado: ${content.titulo.substring(0, 60)}...`);
  ...
  async getStats(req, res) {
    try {
      // MODO SIMULACIÓN
      if (usarDatosSimulados) {
        console.log('\n📊 [getStats] Modo simulación');
        console.log(`   Total: ${todasLasNoticias.length}`);
        console.log(`   Importantes: ${noticiasSimuladas.length}`);
        console.log(`   Publicadas: ${todasLasNoticias.filter(n => n.status === 'published').length}`);
  ...
  async searchContent(req, res) {
    try {
      ...
      if (usarDatosSimulados) {
        ...
        console.log(`\n🔍 [searchContent] "${q}" → ${results.length} resultados`);
